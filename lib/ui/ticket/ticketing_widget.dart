...
import 'package:emembers/services/config_point_service.dart';
...
class ProductListRoute extends PageRouteBuilder {
...
class TicketingListWidget extends StatefulWidget {
  final ProjectList project;
  final User user;
  final String img;
  TicketingListWidget({
  ...
  @override
  _TicketingWidgetState createState() => _TicketingWidgetState();
}

class _TicketingWidgetState extends State<TicketingListWidget>
  ...
  Future<num> fetchConfigPoint() async {
    num configPoint = 0;
    var result = await ConfigPointService().fetchConfigPoint(widget.project);
    if (result.length > 0) {
      configPoint =
          int.parse(result[0].marginalAmount!.toString().replaceAll(".0", ""));
    }
                          ...
                          if (response["returnStatus"] == true) {
                            ...
                            if (dataset.length == 0) {
                              ...
                              if (calculateDifference(bookingDateSelected) ==
                                ...
                                num configPoint = await fetchConfigPoint();
                                Navigator.push(
                                  context,
                                  CheckoutRoute(
                                    widget.project,
